...
def build_cpt_obs(root,train,test,cltr,clte,dic,mest,dclas_prob,classes_order, baseClassifier=rfc()):
	...
	for i in range(0, len(cltr)):
		...
		if(cltr[i,dic[root]]==0):	#si no esta asociado a root
			if(band):		#si es raiz, se agregan los ejemplos negativos (que correponden directamente a los de sus hermanos)
				...
				#reviso si comparten algun padre;
				#spl2=cltr[i].split(p)
	...
	if(c<1 or ndtr<1):
		...
		print("No hay ejemplos suficientes en nodo: "+root)
		print("hay "+str(c)+" de clase negativa")
		print("hay "+str(ndtr)+" de clase positiva")
		#print("Se agregara un ejemplo artificial lleno de +inf en la clase en que hace falta")
		#if(c==0):
		#	att_aux.append(np.array([-1000000 for i in range(0,len(train[0]))]))
		#	cl_aux.append(0)
		#if(ndtr==0):
		#	att_aux.append(np.array([1000000 for i in range(0,len(train[0]))]))
		#	cl_aux.append(1)
	...
	#******************** TEST ***********************************
	#se supone que test ya es una matriz de numpy
	#y dado que las clases vienen en una matriz, debe bastar solo sacarlas
	dtr_t=test	#[]
	dtr_c=clte[:,dic[root]]	#[]
	cpos=0
	cneg=0
	...
	#	#if(spl[0]==""):
	#	if(cltr[i,dic[root]]==1):	#si esta asociado a root
	...
	#imporime cuantos positivos y negativos hay
	#print("pos: "+str(cpos)+", neg:"+str(cneg))
	...
	#	dclas_prob[root]=clf.predict_proba(dtr_t)
	#else:
	#	dclas_prob[root]=np.zeros((len(test),2))
...
# dic diccionario
# mest matriz de estructura
def build_sc(root,train,test,cltr,clte,dic,mest,dclas_prob,classes_order, baseClassifier=rfc()):
	#necesito los valores reales		de test
	#necesito los valores predecidos	de test
	#necesito el modelo del clasificador
	h=0
	...
	#lib print("Accuracy "+root+":"+str(clf.score(dtr_t,dtr_c)))
	cmat=cm(dtr_c,predictions)
	...
	#print(tn, fp, fn, tp)
	#print("")
	#lib print("cpt observaciones "+root)
...
def cpt(root,mest,dic,cltr):
	...
	if(sum(mest[:,dic[root]])<1):
		...
		for i in range(len(cltr)):
			...
			if((sum(arr_index)-arr_index[-1])>=len(fathers)):	#ya que en otro caso no es necesario
...
def HBA(header_in, train_in, test_in, tscore="SP", type_prop="all_probabilities", baseClassifier=rfc()):
	"""
	...
	#path="/home/jonathan/inaoe/PGM/tesis/datasets/FUN2GO/"	#jonathan , o-2016-037-p

	#lib if(len(sys.argv)!=7):
	#lib 	#print("ERROR: you must execute: dag_HBA_v3.py name_dataset m|nm SP|GLB all_probabilities|just_positives")
	#lib 	print("ERROR: you must execute: dag2_HBA_v3.py header_dataset train_data test_data name_out SP|GLB all_probabilities|just_positives")		
	#lib 	exit()
	...
	con2=0.0
	for i in range(0,len(classtr)):	#proporcion de nodos que terminan en varios nodos hoja
		con=0.0
		for x in hojas:
			if(classtr[i,dic[x]]==1):
				con=con+1.0
		if(con>1.0):
			con2=con2+1.0

	#lib print("Proporcion de instancias que terminan en varios nodos hoja:")
	#lib print(str(con2/float(len(classtr))))
	#exit()
			


	nmeasures=7		#numero de medidas de evaluacion programadas 
	...
	for fold in range(0,nfolds): 		#*********************************
		...
		#calcula de forma correcta las cpt_obs, haciendo validacion cruzada de 5 pliegues sobre el conjunto de entrenamiento
		train_co=train.copy()
		...
		for fold_co in range(0,nfolds_co):
			...
			for x in sorted(dic.keys()):
				...
				cptobs_aux[dic[x]],obs[dic[x]],vreal[dic[x]]=build_cpt_obs(x,train_co,test_co,classtr_co,cltest_co,dic,mest,dclas_prob,c_order, baseClassifier)
		...
		#	print("cpt: "+x)
		#	print(cptobs[dic[x]])
		#print("salida despues de las cpt_obs")
		#exit()
		...
		for x in sorted(dic.keys()):
			...
			#rec2(x,data,classes,mest,pathsc+"header_nm.arff",ftest,ctest,mest,dic)
			#build_classifiers(x,data,classes,mest,pathsc+"header_nm.arff",ftest,ctest,mest,dic)
			not_use,obs[dic[x]],vreal[dic[x]]=build_sc(x,train,test,classtr,cltest,dic,mest,dclas_prob,c_order, baseClassifier)		#era build_sc(
			#build_cpt_obs(x,train_co,test_co,classtr_co,cltest_co,dic,mest,dclas_prob,c_order)
			#continue
			cptnodes[dic[x]]=cpt(x,mest,dic,classtr)
			#print(cptnodes[dic[x]])
		...
		#lib print("diccionario de posiciones: "+str(len(dicpos)))
		#lib print(dicpos)
		...
		for x in sorted(dic.keys()):
			marginal = np.sum(prop_values[dicpos[x]], axis=0)		#considerar el numero de padres, un np.sum por cada padre, y revisar en teoria
		...
		#necesito la posición de cada nodo (normales y obasevables) en el vector de values/factors
		...
		for i in range(len(cltest)):
			...
			if(type_prop=="just_positives"):
				...
				for j in sorted(dic.keys()):
					#print("un val de cpt de una ob:")
					#print(cond_values[dicpos[j+"_ob"]][1][1])
					#print("c_order de 1:")
					#print(c_order.index(1))
					#print("prob. de obtener 1")
					#print(dclas_prob[j][i][c_order.index(1)])	#aqui hay un problema
