...
def build_cpt_obs(root,train,test,cltr,clte,dic,dic_inv,mest,dclas,dclas_prob,classes_order,baseClassifier=rfc()):
	...
	l_anc=ancestors(dic[root],mest)		#obtengo la lista de ancestros de root, es vacia para nodos raix, son indices

	ex_pos=[]					#lista de atributos extra todos positivos (solo para ejemplos positivos), no sirve para dag
	ex_real=[]
	for i in range(0,len(l_anc)):
		ex_pos.append(1)
		ex_real.append(0)
	...
	for i in range(0,len(cltr)):
		...
		if(cltr[i,dic[root]]==1):
			if(band):	#si es raiz, simplemente se agregarn los atributos
				...
				#si no es raiz, se agrega la salida de sus ancestros, en este caso (ARBOL) siempre es uno
				#****reanalizar para dag, obtener las saldias reales
				att_aux.append(np.concatenate([train[i], ex_pos ]) )
		...
		if(cltr[i,dic[root]]==0):	#si no esta asociado a root
			if(band):		#si es raiz, se agregan los ejemplos negativos (que correponden directamente a los de sus hermanos)
				...
				if(len([j for j in p if(cltr[i,j]==1) ])>0):	#si comparten almenos un padre, lo agrego	- costoso
					#obtengo los val reales de i, de sus ancestros correspondientes (de root)
					real_ext=[cltr[i,x] for x in l_anc]

					#dtr.append(att[i]+",0")
					#reanalizar la salida real para dag (ex_pos)
		...
		for i in range(0, len(cltr)):
			...
			if(cltr[i,dic[root]]==0):	#si no esta asociado a root
				...
				if(len([j for j in p if(cltr[i,j]==1) ])==0):	#si no comparten padres, lo agrego	- costoso
					...
					if(c>=ndtr):
						break
					#if(len(ex_real)!=len(ex_pos)):
					#	print("diferentes; ex_real: "+str(len(ex_real))+", ex_pos: "+str(len(ex_pos)))
					#	exit()
	...
	if(isroot(root,mest,dic)):
		...
		for i in range(0,len(clte)):
			...
			#dtr_c.append(cltr[i,dic[root]])		#agrega directamente los positivos y negativos
			#spl=clte[i].split(root)
			#if(spl[0]==""):
			#if(cltr[i,dic[root]]==1):	#si no esta asociado a root
			#	#dtr_t.append(att_t[i]+",1")
			#	
...
def HCA(header_in, train_in, test_in, tscore="SP", type_prop="all_probabilities", baseClassifier=rfc()):
	"""
	...
	#lib 	#print("ERROR: you must execute: CA_v3.py name_dataset m|nm SP|GLB all_probabilities|just_positives")
	#lib 	print("ERROR: you must execute: dag2_CA_v3.py header_dataset train_data test_data name_out SP|GLB all_probabilities|just_positives")
	#lib 	exit()
	...
	for fold in range(0,nfolds): 		#*********************************
		...
		for fold_co in range(0,nfolds_co):
			...
			visitados=[]
			#for x in sorted(dic.keys()):	#muy problablemente dara problemas, debe haber un orden en el cual son creadas, por el encadenamiento
			while(len(cola)>0):
				#print(cola)
				x=cola.pop(0)
				...
				if(band):	#si todos los padres de "x" han sido visitados
					...
					#rec2(x,data,classes,mest,pathsc+"header_nm.arff",ftest,ctest,mest,dic)
					#build_classifiers(x,data,classes,mest,pathsc+"header_nm.arff",ftest,ctest,mest,dic)
					cptobs_aux[dic[x]],obs[dic[x]],vreal[dic[x]]=build_cpt_obs(x,train_co,test_co,classtr_co,cltest_co,dic,dic_inv,mest,dclas,dclas_prob,c_order,baseClassifier)
		...
		while(len(cola)>0):
			...
			if(band):	#si todos los padres de "x" han sido visitados
				...
				#rec2(x,data,classes,mest,pathsc+"header_nm.arff",ftest,ctest,mest,dic)
				#build_classifiers(x,data,classes,mest,pathsc+"header_nm.arff",ftest,ctest,mest,dic)
				not_use,obs[dic[x]],vreal[dic[x]]=build_cpt_obs(x,train,test,classtr,cltest,dic,dic_inv,mest,dclas,dclas_prob,c_order,baseClassifier)	#build_sc(x,train,test,classtr,cltest,dic,mest,dclas,dclas_prob,c_order)
				#if(False):
				cptnodes[dic[x]]=cpt(x,mest,dic,classtr)
				visitados.append(x)		#se agrega x a visitados
			else:
