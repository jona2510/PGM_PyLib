...
#devuelve los hijos de un nodo, en una lista
#recibe root, y la matriz estructura 
def childrens(root,mest,dic):
	c=[]	#hijos
	
	for i in range(0,len(mest)):
		if(mest[dic[root],i]==1):
...
def build_cpt_obs(root,train,test,cltr,clte,dic,mest,dclas,dclas_prob,classes_order, baseClassifier=rfc()):
	...
	#dcl=[]
	#ejemplos positivos
	band=isleaf(root,mest,dic)
	root_band=isroot(root,mest,dic)
	ch=childrens(root,mest,dic)	#hijos de root

	arr_aux=np.zeros(len(ch)).astype(int)

	for i in range(0,len(cltr)):
		#spl=cltr[i].split(root)
		#if(spl[0]==""):
		if(cltr[i,dic[root]]==1):
			if(band):	#si es hoja, simplemente se agregarn los atributos
				#dtr.append(att[i]+",1")
				att_aux.append(train[i])				
			else:
				#si no es hoja, se agrega la salida de sus hijos,
				aux=train[i]
				
				for j in range(0,len(ch)):
					#comprobar si el ejemplo pertenece al nodo  "x"
					#spl2=cltr[i].split(x)
					#if(spl2[0]==""):	#si
					#	aux=np.concatenate([aux,[1] ])
					#else:				#no
					#	aux=np.concatenate([aux,[0] ])
					#aux=np.concatenate([aux,cltr[i,dic[x]] ])	#concateno directamente los valores
					arr_aux[j]=cltr[i,dic[ch[j]] ]

				#att_aux.append(aux)
				att_aux.append(np.concatenate([train[i], arr_aux ]) )
	...
	out_neg=np.zeros(len(ch)).astype(int)	#[]
	...
	for i in range(0, len(cltr)):
		...
		if(cltr[i,dic[root]]==0):	#si no esta asociado a root
			if(band):		#si es hoja, se agregan los ejemplos negativos (que correponden directamente a los de sus hermanos)
				#tengo que buscar los ejemplos negativos de sus hermanos del nodo
				if(root_band):	#si es root, se agregan directamente los negativos
					#dtr.append(att[i]+",0")
					att_aux.append(train[i])
					cl_aux.append(0)
					c=c+1
					if(c>=ndtr):
						break
				else:	#si es hoja y no es nodo raiz, entonces se agregan ejemplos negativos con el mismo padre
					#spl2=cltr[i].split(p)
					...
					if(len([j for j in p if(cltr[i,j]==1) ])>0):	#si comparten almenos un padre, lo agrego	- costoso
						#dtr.append(att[i]+",0")
						#todos lo negativos tienen a todos lo hijos con ceros :/					
						#att_aux.append(np.concatenate([train[i],out_neg] ))					
						att_aux.append(train[i])
						cl_aux.append(0)
						c=c+1
				...
				if(root_band):		#no es hoja, pero es raiz, por lo agrega los ejemplos negativos (con sus hijos), 
					att_aux.append(np.concatenate([train[i],out_neg] ))	# si no esta asociada a root, por RJ no esta asociada a los hijos de root
					cl_aux.append(0)
					c=c+1
					if(c>=max_nb):	#era ndtr, se cambia por el porcentaje
						break
	...
	if(c<ndtr and not root_band):		#si no hubo suficientes ejemplos negativos de sus hermanos
		for i in range(0, len(cltr)):
			#spl=cltr[i].split(root)
			#if(spl[0]!=""):
			if(cltr[i,dic[root]]==0):	#si no esta asociado a root
	...
	#aqui pregunto si es root, si es root se deja normal, si no, se agrega como atributo la salida de sus hijos, en dclas
	dtr_t=[]
	dtr_c=clte[:,dic[root]]	#[]
	cpos=0
	cneg=0
	...
	if(band):	#si es hoja
		...
		for i in range(0,len(clte)):
			...
			aux=[]
			for x in ch:
				aux.append(dclas[x][i])
			dtr_t.append(np.concatenate([test[i],aux ]) )		#concateno la salida del padre para el i-esimo elemento
	...
	#suavizado laplaciano	no suma en f,f
	#fp=fp+1
	#tn=tn+1
	#fn=fn+1
	#tp=tp+1
...
#devuelve el numero de hermanos de cada nodo
def n_siblings(mest):
...
def HCC(header_in, train_in, test_in, tscore="SP", type_prop="all_probabilities", baseClassifier=rfc()):
	"""
	...
	#lib 	#print("ERROR: you must execute: CLR_v3.py name_dataset m|nm SP|GLB all_probabilities|just_positives")
	#lib 	print("ERROR: you must execute: dag2_CLR_v3.py header_dataset train_data test_data name_out SP|GLB all_probabilities|just_positives")
	#lib	exit()
	...
	dic_inv={}
	cl_wop=""
	#with open(pathsc+"header_"+monm+".arff","r") as f:
	...
	data=[]
	classes=[]
	#with open(pathsc+"data_"+monm+".arff","r") as f:
	...
	for fold in range(0,nfolds): 		#*********************************
		...
		for fold_co in range(0,nfolds_co):
			...
			########
			cola=hojas.copy()
			visitados=[]	#nodos que ya han sido visitados
			while(len(cola)>0):
				#print("cola: ")
				#print(cola)
				x=cola.pop(0)		#extraigo el primer elemento de la cola
			
				ch=childrens(x,mest,dic)
				band=True
				for z in ch:
				...
				if(band):	#si todos los hijos de "x" han sido visitados
					#agrego los padres de "x" a la cola
					#	comprobando que no esten en la cola
					for i in range(0,len(mest)):
						#if(mest[dic[x],i]==1):
						if(mest[i,dic[x]]==1):
							#obtengo el nombre del nodo
							aux=dic_inv[i]	#[r[0] for r in dic.items() if(r[1]==i)]
							#agrego
							#if(len(aux)>0):
							if(not aux in cola and not aux in visitados):	#modificado 
								cola.append(aux)
					...
					cptobs_aux[dic[x]],obs[dic[x]],vreal[dic[x]]=build_cpt_obs(x,train_co,test_co,classtr_co,cltest_co,dic,mest,dclas,dclas_prob,c_order, baseClassifier)
					#agrega a cptobs
				...
				else:	#si hijos de "x" no han sido visitados, x es regresado a la cola
					cola.append(x)
		...
		while(len(cola)>0):
			...
			if(band):	#si todos los hijos de "x" han sido visitados
				...
				#rec2(x,data,classes,mest,pathsc+"header_nm.arff",ftest,ctest,mest,dic)
				#build_classifiers(x,data,classes,mest,pathsc+"header_nm.arff",ftest,ctest,mest,dic)
				not_use,obs[dic[x]],vreal[dic[x]]=build_cpt_obs(x,train,test,classtr,cltest,dic,mest,dclas,dclas_prob,c_order, baseClassifier)	#build_sc(x,train,test,classtr,cltest,dic,mest,dclas,dclas_prob,c_order)
				#if(False):
